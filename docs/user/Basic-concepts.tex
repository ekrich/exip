...
To start with, the XML to EXI conversion requires a XML parser to process the XML input.
The XML parser itself is at least as big a chunk of code as the EXI parser. Having them both at the
same time might not be possible or desired on a resource constrained embedded system. Second, parsing
...
The EXIP library is an C project and supports Windows via Visual Studio 2022, Linux via the GCC toolchain,
and macOS via the Clang toolchain. The project is structured in the following subdirectories:
\begin{itemize}
...
provides a Microsoft Visual Studio 2022 build under \texttt{build/vs2022/} and \emph{GCC} toolchain builds
under \texttt{build/gcc/}. The \texttt{build/gcc/} folder further contains platform-dependent parameters and
headers located in subfolders - \texttt{build/gcc/zoul} for the Contiki-ng sensor platform OS and
\texttt{build/gcc/pc} for desktop PCs. The \texttt{build/gcc/contiki} is the legacy Contiki platform.
The build is started from \texttt{build/gcc/} with the following \texttt{make} build targets available:
\begin{description}
 \item[all] compiles the EXIP library. The default target platform is PC or \texttt{TARGET=pc}. To change
the target platform to Contiki-ng for example, add \texttt{TARGET=zoul} as an argument of \texttt{make}
 \item[check] invokes the Check unit tests
 \item[clean] removes the \texttt{bin/} folder
 \item[docs] generates the project Doxygen documentation. The output directory is set to \texttt{docs/dev/doxygen/}
 ...
 \item \texttt{docs/} - the project documentation: \texttt{docs/user/} contains the source of this user guide;
    \texttt{docs/dev/} is the development documentation;
  \texttt{docs/www/} contains the information available on the project web page
...
\texttt{build/vs2022} for Windows.
