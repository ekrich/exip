...
by the \texttt{exipg} utility implemented in \texttt{utils/schemaGen/createGrammars.c}.
...
\begin{lstlisting}
exipg -static -pfx=ops_ -ops=0001000 -schema=utils/schemaGen/EXIOptions-xsd.exi src/contentIO/src/staticEXIOptions.c
\end{lstlisting}
...
The latest distribution of the EXIP library doesn't includes a convenient GUI tool
that can be used to encode XSD schema files into EXI representation. A Java GUI tool from EXIficient can be found
at the following location: \href{https://exificient.github.io/java/}{(https://exificient.github.io/java/)}.
Download the runnable JAR. The graphical tool can be started with the following command:
\begin{lstlisting}
java -jar exificient-gui-jar-with-dependencies.jar
\end{lstlisting}
When encoding the XML Schema file, configure the following settings:
\begin{itemize}
\item Alignment \& StringTable: \textbf{BIT\_PACKED} (default)
\item Schema Information: \textbf{Schema-less} (default)
\item Coding Options: \textbf{Preserve Prefixes} (required - to map elements to built in XML Schema namespace)
\item Click \textbf{Configure Advanced Encoding Options} button:
\begin{itemize}
\item EXI Header: \textbf{Include EXI options} (recommended - out-of-band options not needed)
\end{itemize}
\item Select the \textbf{XML to EXI} tab (default)
\item Select XML file (input XSD file)
\item EXI output file (output .exi file)
\end{itemize}

If you leave ``Include EXI options'' unchecked, you need to use the \texttt{-ops=0001000} argument
of the \texttt{exipg utility} to specify the out-of-band options (bit 4 enables Preserve Prefixes).

Note: This formerly used the OpenEXI Java library found at
\url{https://sourceforge.net/projects/openexi/files/}.
