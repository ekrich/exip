...
\begin{itemize}
\item Alignment: \textbf{BIT\_PACKED} (default)
\item \textbf{Preserve Prefixes} (must check this box)
\item Schema Information: \textbf{Schema-less} (default)
\item Click \textbf{Configure Advanced Encoding Options} button:
\begin{itemize}
\item \textbf{Include cookie} (includes EXI header with cookie)
\item \textbf{Include options} (embeds options in EXI header)
\end{itemize}
...
If you leave ``Include cookie'' and ``Include options'' unchecked, you need to use the \texttt{-ops=0001000} argument
of the exipg utility to specify the out-of-band options (bit 4 enables Preserve.prefixes).
